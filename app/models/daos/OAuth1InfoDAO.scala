...
class OAuth1InfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def oAuth1InfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOAuth1Info <- slickOAuth1Infos if dbOAuth1Info.loginInfoID === dbLoginInfo.loginInfoID
  } yield dbOAuth1Info
  ...
  protected def oAuth1InfoSubQuery(loginInfo: LoginInfo) =
    slickOAuth1Infos.filter(_.loginInfoID in loginInfoQuery(loginInfo).map(_.loginInfoID))

  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth1Info) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      slickOAuth1Infos += DBOAuth1Info(UUID.randomUUID(), authInfo.token, authInfo.secret, dbLoginInfo.loginInfoID)
    }.transactionally
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth1Info): Future[OAuth1Info] = {
    val query = loginInfoQuery(loginInfo).joinLeft(slickOAuth1Infos).on(_.loginInfoID === _.loginInfoID)
