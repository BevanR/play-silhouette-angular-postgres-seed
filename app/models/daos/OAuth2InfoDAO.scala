...
class OAuth2InfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def oAuth2InfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOAuth2Info <- slickOAuth2Infos if dbOAuth2Info.loginInfoID === dbLoginInfo.loginInfoID
  } yield dbOAuth2Info
  ...
  protected def oAuth2InfoSubQuery(loginInfo: LoginInfo) =
    slickOAuth2Infos.filter(_.loginInfoID in loginInfoQuery(loginInfo).map(_.loginInfoID))
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val query = for {
      result <- loginInfoQuery(loginInfo).joinLeft(slickOAuth2Infos).on(_.loginInfoID === _.loginInfoID)
    } yield result
