...
class OpenIDInfoDAO @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def openIDInfoQuery(loginInfo: LoginInfo) = for {
    ...
    dbOpenIDInfo <- slickOpenIDInfos if dbOpenIDInfo.loginInfoID === dbLoginInfo.loginInfoID
  } yield dbOpenIDInfo

  protected def addAction(loginInfo: LoginInfo, authInfo: OpenIDInfo) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      DBIO.seq(
        slickOpenIDInfos += DBOpenIDInfo(authInfo.id, dbLoginInfo.loginInfoID),
  ...
  def save(loginInfo: LoginInfo, authInfo: OpenIDInfo): Future[OpenIDInfo] = {
    val query = loginInfoQuery(loginInfo).joinLeft(slickOpenIDInfos).on(_.loginInfoID === _.loginInfoID)
  ...
  def remove(loginInfo: LoginInfo): Future[Unit] = {
    ...
    val openIDInfoSubQuery = slickOpenIDInfos.filter(_.loginInfoID in loginInfoQuery(loginInfo).map(_.loginInfoID))
